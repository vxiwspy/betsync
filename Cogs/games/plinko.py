...
class PlinkoCog(commands.Cog):
    ...
    def generate_plinko_image(self, rows, path, landing_position, multipliers):
        ...
        # Dimensions - dynamically adjust for higher row counts
        base_width = 800
        base_height = 800

        # Scale down for higher row counts
        if rows > 16:
            scale_factor = 16 / rows
            width = base_width
            height = int(base_height / scale_factor)
            peg_radius = max(4, int(10 * scale_factor))  # Minimum peg size of 4
            ball_radius = max(6, int(15 * scale_factor))  # Minimum ball size of 6
        else:
            width = base_width
            height = base_height
            peg_radius = 10
        ...
        # Add the multiplier values at the bottom - scale font size for higher row counts
        font_size = 24 if rows <= 16 else max(12, int(24 * (16 / rows)))
        multiplier_font = ImageFont.truetype("roboto.ttf", font_size)
        bucket_width = width / len(multipliers)

        for i, multiplier in enumerate(multipliers):
            x = bucket_width * (i + 0.5)
            y = height - (30 if rows <= 16 else 15)
            ...
            # For very high row counts, round to 1 decimal place to save space
            if rows > 20 and len(multipliers) > 20:
                multiplier_text = f"{round(multiplier, 1)}x"
